...
namespace Microsoft.AspNet.OData
{
    ...
    public partial class EnableQueryAttribute
    {
            ...
            HttpRequestMessage request)
        {
            ...
            if (request != null)
            {
                // shouldn't need concurrent dictionary here since this is per-request
                object queryContexts;
                if(!request.Properties.TryGetValue("QueryContexts", out queryContexts))
                {
                    queryContexts = new List<ODataQueryContext>();
                    request.Properties.Add("QueryContexts", queryContexts);
                }
                ((IList<ODataQueryContext>)queryContexts).Add(queryContext);
            }
