...
namespace Microsoft.Test.E2E.AspNet.OData.Batch.Tests.DataServicesClient
{
    ...
    public class DefaultBatchOrderController : InMemoryODataController<DefaultBatchOrder, int>
    {
        ...
        protected override Task<ITestActionResult> CreateEntityAsync(DefaultBatchOrder entity)
        {
            if (entity.Id < 0)
            {
                return Task.FromResult(BadRequest() as ITestActionResult);
...
");
        ...
        {
            // Arrange
            string requestUri = string.Format("{0}/DefaultBatch/$batch", this.BaseAddress);
            string defaultBatchCustomersAbsoluteUri = string.Format("{0}/DefaultBatch/DefaultBatchCustomer", this.BaseAddress);
            string defaultBatchOrdersAbsoluteUri = string.Format("{0}/DefaultBatch/DefaultBatchOrder", this.BaseAddress);
...
");
        ...
        public async Task CanHandleReferencingOfRequestsNotSharingAutomicityGroup()
        {
            // Arrange
            string requestUri = string.Format("{0}/DefaultBatch/$batch", this.BaseAddress);
            string defaultBatchCustomersRelativeUri = "DefaultBatchCustomer";
            string defaultBatchOrdersRelativeUri = "DefaultBatchOrder";
