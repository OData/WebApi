...
namespace WebStack.QA.Test.OData.Formatter.JsonLight.Metadata
{
    ...
    public class EntryMetadataTests
    {
        ...
        protected static IEdmModel GetCustomNavigationPropertyConventionsModel(HttpConfiguration config)
        {
            ODataModelBuilder builder = new ODataConventionModelBuilder(config);
            var oneToOneChildSet = builder.EntitySet<OneToOneChild>("OneToOneChild");
            var oneToOneParentSet = builder.EntitySet<OneToOneParent>("OneToOneParent");
        ...
        protected static IEdmModel GetCustomReadLinkConventionsModel(HttpConfiguration config)
        {
            ODataModelBuilder builder = new ODataConventionModelBuilder(config);
            var oneToOneParentSet = builder.EntitySet<OneToOneParent>("OneToOneParent");
            oneToOneParentSet.EntityType.Ignore(x => x.Child);
            oneToOneParentSet.HasReadLink(eic => "http://localhost:5000/CustomReadLink", followsConventions: false);
            return builder.GetEdmModel();
        }

        protected static IEdmModel GetCustomEditLinkConventionsModel(HttpConfiguration config)
        {
            ODataModelBuilder builder = new ODataConventionModelBuilder(config);
            var oneToOneParentSet = builder.EntitySet<OneToOneParent>("OneToOneParent");
            oneToOneParentSet.EntityType.Ignore(x => x.Child);
            oneToOneParentSet.HasEditLink(eic => "http://localhost:5000/CustomEditLink", followsConventions: false);
            return builder.GetEdmModel();
        }
        ...
        private static IEdmModel GetCustomIdLinkConventionsModel(HttpConfiguration config)
        {
            ...
            var oneToOneParentSet = builder.EntitySet<OneToOneParent>("OneToOneParent");
            oneToOneParentSet.EntityType.Ignore(x => x.Child);
            oneToOneParentSet.HasIdLink(eic => "http://localhost:5000/CustomIdLink", followsConventions: false);
            return builder.GetEdmModel();
        }
        ...
        public void NavigationLinksAppearOnlyInFullMetadata(string acceptHeader)
        {
            ...
            {
                JsonAssert.Equal(expectedNavigationLinkUrl, "Child@odata.navigationLinkUrl", result);
            }
            else
            {
        ...
        public void CustomEditLinksAppearInFullAndMinimalMetadata(string acceptHeader)
        {
            ...
            if (!acceptHeader.Contains("nometadata"))
            {
                JsonAssert.Equal(expectedEditLinkUrl, "odata.editLink", result);
            }
            else
            {
        ...
        public void CustomIdLinksAppearInFullAndMinimalMetadata(string acceptHeader)
        {
            ...
            if (!acceptHeader.Contains("nometadata"))
            {
                JsonAssert.Equal(expectedEditLinkUrl, "odata.id", result);
            }
            else
            {
        ...
        public void CanFollowGeneratedNavigationLinks(string acceptHeader)
        {
            ...
            {
                string childUrl = (string)returnedParentEntities[i]["Child@odata.navigationLinkUrl"];
