...
namespace Microsoft.Test.E2E.AspNet.OData.BulkOperation
{
    public class BulkOperationTest : WebHostTestBase
    {
        ...
        [Fact]
        public async Task PatchEmployee_WithEmployeeODataIdFriendThenDeleteLinkedFriend()
        {
            //Arrange
            string requestUri = this.BaseAddress + "/convention/Employees(1)";
            ...
            var content = @"{
                    'Name':'Sql'  ,
                    'Friends@odata.delta':[{ '@odata.id' : 'Employees(2)/Friends(3)'}]
                     }";
            ...
            var expected = "{\"Id\":3,\"Name\":\"Test3\",\"Age\":0}";
            ...
            using (HttpResponseMessage response = await this.Client.GetAsync(requestUri))
            {
                ...
                var res = await response.Content.ReadAsObject<JObject>();
                var result = res.GetValue("value") as JArray;
            ...
            content = @"{
                    'Name':'Sql'  ,
                    'Friends@odata.delta':[{ '@odata.removed' : {'reason':'changed'}, 'Id':3}]
                     }";

            requestForPost = new HttpRequestMessage(new HttpMethod("PATCH"), requestUri);
            requestForPost.Headers.Add("OData-Version", "4.01");

            stringContent = new StringContent(content: content, encoding: Encoding.UTF8, mediaType: "application/json");
            requestForPost.Content = stringContent;
            ...
            using (HttpResponseMessage response = await this.Client.GetAsync(requestUri))
            {
                ...
                Assert.DoesNotContain("Test3", result.ToString());
                Assert.DoesNotContain(expected.ToLower(), json.ToString().ToLower());
            }
        }
