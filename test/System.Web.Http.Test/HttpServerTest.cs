...
namespace System.Web.Http
{
    public class HttpServerTest
    {
        ...
        [Fact, RestoreThreadPrincipal]
        public void SendAsync_SetsThreadPrincipalOnRequestContext_WhenNull()
        {
            IPrincipal expectedPrincipal = new Mock<IPrincipal>().Object;
            ...
            using (HttpRequestMessage request = new HttpRequestMessage())
            {
                ...
                mockHandler.Protected()
                    .Setup<Task<HttpResponseMessage>>("SendAsync", request, CancellationToken.None)
                    .Callback(() => principal = requestContext.Principal)
                    .Returns(TaskHelpers.FromResult<HttpResponseMessage>(null));
                HttpMessageHandler handler = mockHandler.Object;

                using (HttpServer server = new HttpServer(handler))
                using (HttpMessageInvoker invoker = new HttpMessageInvoker(server))
                {
                    // Act
        ...
        [Fact, RestoreThreadPrincipal]
        public void SendAsync_DosNotChangeExistingThreadPrincipalOnRequestContext()
        {
            IPrincipal expectedPrincipal = new Mock<IPrincipal>().Object;
            IPrincipal principal = null;
            ...
            using (HttpRequestMessage request = new HttpRequestMessage())
            {
                ...
                Thread.CurrentPrincipal = new Mock<IPrincipal>().Object;
